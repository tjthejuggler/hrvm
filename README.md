...
- **Coherence Score Algorithm Improvements:** *(2026-02-23)* Four improvements to `calculate_coherence_score()` in `math_utils.py`: **(1) Band power integration** — the target-band power is now computed by summing all PSD bins within ±0.03 Hz of the target frequency (previously only the single peak bin was used; at 4 Hz / 256 samples the band spans ~4 bins, so summing captures energy smeared across adjacent bins when breathing rate doesn't align exactly with a bin). **(2) Honest periodogram** — replaced `scipy.signal.welch(nperseg=N)` with `scipy.signal.periodogram()`, which is mathematically identical but makes the intent explicit (single-segment, no Welch averaging). **(3) 64-second / 256-sample window** — the rolling HR interpolation window was extended from 60 s (240 samples) to 64 s (256 samples = 2⁸), giving the FFT its optimal power-of-2 input length and slightly improving frequency resolution (0.0156 Hz/bin vs 0.0167 Hz/bin). **(4) Pre-interpolation artifact rejection** — a new `reject_rr_artifacts()` function in `math_utils.py` filters RR intervals before PCHIP interpolation using absolute bounds (333–1500 ms = 40–180 BPM) and a 20% relative-change gate between consecutive beats (standard ectopic-beat rejection). The RR buffer admission check in `signal_processor.py` was also tightened from 300–2000 ms to 333–1500 ms to match.
- **HRV Calculation Correctness Fixes:** *(2026-02-27)* Six clinical-accuracy fixes applied across `src/processing/math_utils.py` and `src/processing/signal_processor.py` following an external review: **(1) SDNN sample std dev** — `calculate_sdnn()` now uses `np.std(ddof=1)` (divides by N−1) per the Task Force of the European Society of Cardiology (1996) standard; previously used population std dev (÷N). **(2) SDNN 5-minute buffer** — SDNN is now computed on a dedicated rolling 5-minute window (`sdnn_buffer`) in `signal_processor.py`; RMSSD continues to use the 60-second window. SDNN over 60 s is not clinically meaningful. **(3) Artifact-aware RMSSD** — a new `calculate_rmssd_artifact_aware()` function computes successive differences on the full RR array, then drops any difference where either beat is an artifact. Previously, artifacts were deleted from the array before `np.diff()`, causing non-adjacent beats to be paired across time gaps. **(4) Rolling-median artifact rejection** — `reject_rr_artifacts()` now compares each beat against the rolling median of the last 9 valid beats (instead of the strict i−1 neighbour), preventing the "domino effect" where a single ectopic beat causes the following normal beat to also be rejected. **(5) Coherence PSD on RR (ms) not BPM** — `interpolate_rr_stream()` (new function) interpolates RR intervals in milliseconds onto the uniform time grid; `calculate_coherence_score()` now receives RR ms, matching the clinical standard (PSD in ms²/Hz). `interpolate_hr_stream()` is retained for display-only BPM output. **(6) Coherence frequency floor** — the total-power lower bound in `calculate_coherence_score()` was corrected from 0.0033 Hz (VLF, physically impossible to resolve in a 64-second window) to 0.015 Hz (= 1/64 Hz, the lowest resolvable frequency). Zero-division is now protected by `max(denominator, 1e-9)` instead of a conditional branch.
...
- **Assessment Quality Gates:** *(2026-02-23)* Both the stepped and continuous assessment protocols now include minimum quality thresholds to prevent false resonance frequency results from low-quality sessions. **Stepped protocol:** each epoch is flagged as invalid (`⚠`) if phase synchrony < 0.25 or PT amplitude < 1.5 BPM — the score is still recorded but shown in amber with a warning marker in the leaderboard and history tab. **Continuous protocol:** the result is declared **Inconclusive** (not saved) if the peak PLV never exceeds 0.3 (HR phase and breath phase were essentially uncorrelated throughout the session) or if the resonance index curve is too flat (peak < 1.5× session mean, indicating no clear resonance peak — just noise). The UI shows an orange "ASSESSMENT INCONCLUSIVE" message with instructions to retry in a calmer state.
