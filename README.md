...
- **ACC-Based Respiration Detection:** *(2026-02-22)* Automatic real-time breathing phase detection (INHALE / EXHALE / HOLD) derived from the Polar H10 accelerometer Z-axis. A dedicated **Breathing Phase** chart in the Polar H10 graphs section shows the current phase as a step-line with colour coding. The Polar H10 top bar includes a **Breath Profile** dropdown (standing / sitting / laying) and a **Calibrate Breath** button. Clicking "Calibrate Breath" opens a popup where you hold `Z` (inhale), `X` (exhale), or `C` (hold) to teach the system your personal thresholds for the selected posture. Calibration data is saved to `acc_breath_cal.json` and reloaded automatically on next launch. The detected breath rate (br/min) is shown live in the top bar.
